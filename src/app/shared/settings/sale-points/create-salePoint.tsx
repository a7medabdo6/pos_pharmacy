...
import {
  SalePointFormInput,
  salePointFormSchema,
} from '@/utils/validators/create-sale-point.schema';
...
export default function Createsalepoint({
  ...
  id?: string;
  isModalView?: boolean;
  category?: SalePointFormInput;
}) {
  const [reset, setReset] = useState({});
  const { mutate: Create, isLoading } = useCreateUnitHook();
  const onSubmit: SubmitHandler<SalePointFormInput> = (data) => {
    Create(data);
    // setTimeout(() => {
  ...
  return (
    <Form<SalePointFormInput>
      validationSchema={salePointFormSchema}
      resetValues={reset}
      onSubmit={onSubmit}
      useFormProps={{
        mode: 'onChange',
          ...
          <div className="flex-grow pb-10">
            ...
            >
              <HorizontalFormBlockWrapper
                title={'Add new sale point:'}
                description={'Edit your sale point information from here'}
